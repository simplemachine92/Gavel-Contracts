...
interface IEuler {
    ...
    function moduleIdToImplementation(uint256 moduleId)
        external
        view
        returns (address);
...
interface IEulerMarkets {
    ...
    function underlyingToAssetConfig(address underlying)
        external
        view
        returns (IEuler.AssetConfig memory);
    ...
    function getChainlinkPriceFeedConfig(address underlying)
        external
        view
        returns (address chainlinkAggregator);
...
interface IEulerExec {
    ...
    function liquidity(address account)
        external
        view
        returns (LiquidityStatus memory status);
    ...
    function detailedLiquidity(address account)
        external
        view
        returns (AssetLiquidity[] memory assets);
    ...
    function batchDispatchSimulate(
        EulerBatchItem[] calldata items,
        address[] calldata deferLiquidityChecks
    ) external;
    ...
    /// @param account User account (xor in subAccountId, if applicable)
    /// @return The average liquidity, in terms of the reference asset, and post risk-adjustment
    function getAverageLiquidityWithDelegate(address account)
        external
        returns (uint256);
    ...
    function getAverageLiquidityDelegateAccount(address account)
        external
        view
        returns (address);
    ...
    function doStaticCall(address contractAddress, bytes memory payload)
        external
        view
...
interface IEulerDToken {
    ...
    /// @param holder Xor with the desired sub-account ID (if applicable)
    /// @param spender Trusted address
    function debtAllowance(address holder, address spender)
        external
        view
        returns (uint256);
...
interface IEulerSwap {
    ...
    function swapUniExactInputSingle(
        SwapUniExactInputSingleParams memory params
    ) external;
    ...
    /// @param params struct defining trade parameters
    function swapUniExactOutputSingle(
        SwapUniExactOutputSingleParams memory params
    ) external;

    /// @notice Execute Uniswap V3 exact output trade routed through multiple pools
    /// @param params struct defining trade parameters
    function swapUniExactOutput(SwapUniExactOutputParams memory params)
        external;
    ...
    function swapAndRepayUniSingle(
        SwapUniExactOutputSingleParams memory params,
        uint256 targetDebt
    ) external;
...
interface IEulerEulStakes {
    ...
    function staked(address account, address underlying)
        external
        view
        returns (uint256);
...
library EulerAddrsMainnet {
    ...
    IEulerLiquidation public constant liquidation =
        IEulerLiquidation(0xf43ce1d09050BAfd6980dD43Cde2aB9F18C85b34);
    IEulerExec public constant exec =
        IEulerExec(0x59828FdF7ee634AaaD3f58B19fDBa3b03E2D9d80);
    IEulerSwap public constant swap =
        IEulerSwap(0x7123C8cBBD76c5C7fCC9f7150f23179bec0bA341);
}

library EulerAddrsGoerli {
    IEuler public constant euler =
        IEuler(0x931172BB95549d0f29e10ae2D079ABA3C63318B3);
    IEulerMarkets public constant markets =
        IEulerMarkets(0x3EbC39b84B1F856fAFE9803A9e1Eae7Da016Da36);
    IEulerLiquidation public constant liquidation =
        IEulerLiquidation(0x66326c072283feE63E1C3feF9BD024F8697EC1BB);
    IEulerExec public constant exec =
        IEulerExec(0x4b62EB6797526491eEf6eF36D3B9960E5d66C394);
